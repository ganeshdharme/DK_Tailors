...
public class JFXMLController_NewCustomer implements Initializable
{
	...
	private boolean ValidateCustomerEntry()
	{
		...
		{
			ShowAlert("WARNING!", "Some of the necessary fields are empty!");
			return false;
		}
		...
		{
			ShowAlert("WARNING!", "Mobile number should be a numerical value!");
			return false;
		}

		if (String.valueOf(mobNo).length() != 10)
		{
			ShowAlert("WARNING!", "Mobile number should be 10 digit long!");
			return false;
		}
		...
		if (!email.isEmpty())
		{
			...
			{
				ShowAlert("WARNING!", "Email Address is Invalid!");
				return false;
			}
		}
